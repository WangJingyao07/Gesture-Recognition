...
while 1:
    ...
    for i in range(nPoints):
        ...
        if prob > threshold :
            ...
            # point[0] = signal.savgol_filter(point[0], window_length, polyorder)
            # point[1] = signal.savgol_filter(point[1], window_length, polyorder)
            # data.append(x_data + y_data)
            # for i in range(43): data[str(i)] = signal.savgol_filter(data[str(i)], window_length, polyorder)
    ...
    for pair in POSE_PAIRS:
        partA = pair[0]
        partB = pair[1]
        x_data.append(partA[0])
        y_data.append(partA[1])
    ...
    df.to_csv('HandData2.csv', index=False)
